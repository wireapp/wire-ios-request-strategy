...
class UpdateRoleActionHandler: ActionHandler<UpdateRoleAction> {
    override func request(for action: UpdateRoleAction) -> ZMTransportRequest? {
        guard
        ...
        let request = ZMTransportRequest(path: path, method: .methodPUT, payload: payloadString as ZMTransportData, apiVersion: APIVersion.v0.rawValue)
        return request
    }
