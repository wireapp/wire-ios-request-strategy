...
class UpdateEventsStoreMigrationTests: MessagingTestBase {
    ...
    var previousEventStoreLocations : [URL] {
        return [
            sharedContainerURL,
            sharedContainerURL.appendingPathComponent(accountIdentifier.uuidString)
...
//    }
//    
//    fileprivate func removeFilesInSharedContainer() {
...
//        // given
//        StorageStack.shared.createStorageAsInMemory = false
//        let eventMOC_sameLocation = NSManagedObjectContext.createEventContext(withSharedContainerURL: sharedContainerURL, userIdentifier: accountIdentifier)
...
class StoreUpdateEventTests: MessagingTestBase {
    ...
    func testThatYouCanCreateAnEvent() {
        ...
        self.eventMOC.performGroupedBlockAndWait {
            ...
            // when
            if let storedEvent = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 2) {
    ...
    func testThatItFetchesAllStoredEvents() {
        ...
        self.eventMOC.performGroupedBlockAndWait {
            ...
            guard let storedEvent1 = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 0),
                let storedEvent2 = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 1),
                let storedEvent3 = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 2)
                else {
                    return XCTFail("Could not create storedEvents")
            }
    ...
    func testThatItOrdersEventsBySortIndex() {
        ...
        self.eventMOC.performGroupedBlockAndWait {
            ...
            guard let storedEvent1 = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 0),
                let storedEvent2 = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 30),
                let storedEvent3 = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 10)
                else {
                    return XCTFail("Could not create storedEvents")
            }
    ...
    func testThatItReturnsOnlyDefinedBatchSize() {
        ...
        self.eventMOC.performGroupedBlockAndWait {
            ...
            guard let storedEvent1 = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 0),
                let storedEvent2 = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 10),
                let storedEvent3 = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 30)
                else {
                    return XCTFail("Could not create storedEvents")
            }
    ...
    func testThatItReturnsHighestIndex() {
        ...
        self.eventMOC.performGroupedBlockAndWait {
            ...
            guard let _ = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 0),
                let _ = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 1),
                let _ = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 2)
                else {
                    return XCTFail("Could not create storedEvents")
            }
    ...
    func testThatItCanConvertAnEventToStoredEventAndBack() {
        ...
        self.eventMOC.performGroupedBlockAndWait {
            ...
            // when
            guard let storedEvent = StoredUpdateEvent.create(event, managedObjectContext: self.eventMOC, index: 0)
                else {
                    return XCTFail("Could not create storedEvents")
