...
- (void)setUp {
    ...
    self.coreDataStack = self.coreDataStack = [self createCoreDataStackWithUserIdentifier:[NSUUID UUID]
                                                                            inMemoryStore:YES];
    self.dummyRequest = [ZMTransportRequest requestGetFromPath:self.name apiVersion:v0];
    self.dummyResponse = [ZMTransportResponse responseWithPayload:nil HTTPStatus:200 transportSessionError:nil apiVersion:v0];
...
- (void)testThatItReturnsTheInitParameters
{
    ...
    // then
    XCTAssertEqual([sut nextRequestForAPIVersion:v0], self.dummyRequest);
...
- (void)testThatItDoesNotReturnTheRequestIfTheTimerIsNotExpired
{
    ...
    // when
    ZMTransportRequest *request1 = [sut nextRequestForAPIVersion:v0];
    ...
    for(int i = 0; i < ATTEMPTS; ++i) {
        ...
        ZMTransportRequest *request2 = [sut nextRequestForAPIVersion:v0];
        // then
...
- (void)testThatItCallsNextRequestAfterATimeout
{
    ...
    // when
    ZMTransportRequest *request = [sut nextRequestForAPIVersion:v0];
...
- (void)testThatItStopsCallingNextRequestWhenCanceled
{
    ...
    [sut nextRequestForAPIVersion:v0];
    [sut invalidate];
